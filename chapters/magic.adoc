...
En termes de puissance, un tour de magie reproduit l'effet d'un élément d'équipement standard, peu complexe, sans personnalisation ni option, d'une valeur maximale de 500¥ environ.
L'effet reproduit _ne peut entraîner aucun effet mécanique_ : un tour de magie ne peut occasionner aucun dommage direct, n'entraîner aucune jet de dés, ne modifier aucun indice, ne donner aucun modificateur, et ainsi de suite.
...
====== Combat
*Chasse Insectes :* Tue ou éloigne les petits insectes normaux dans un rayon de 50cm autour du lanceur. +
...
====== Illusion
*Hologramme :* Le lanceur crée entre ses mains une réprésentation en volume de son choix. La représentation doit tenir dans un cube de [.formula]#<<attribute_magic,Magie>> ×10# centimètres de coté, et sa qualité est limitée par la compétence artistique et, le cas échéant, la mémoire du lanceur. +
