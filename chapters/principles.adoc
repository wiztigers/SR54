...
S'il n'y a vraiment aucune <<chapter_skills,compétence>> ni <<knowledges,connaissance>> reflétant la nature de la tâche entreprise, il est possible de résoudre une situation avec un test d'attribut : la réserve de dés à utiliser est alors égale à *attribut × 2* (au lieu de *attribut + compétence*).

Par exemple, il peut être nécessaire de savoir si le personnage parvient à se rappeler un détail d'une situation qu'il a vécu (via un test de mémoire utilisant son Intelligence × 2), ou s'il a le réflexe de rattraper un objet in extremis (en utilisant sa Réaction × 2) ...
