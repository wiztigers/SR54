...
Cela peut venir par exemple de l'environnement extérieur, des blessures affectant le personnage, de la résistance passive d'un sujet, et ainsi de suite.
...
Différents tests peuvent se voir assortis de différents modificateurs, suivant leur nature.
Ces modificateurs sont détaillés à travers les différents chapitres de ce livre, mais constituent toujours de simples applications des règles suivantes :
...
En conséquence, l'action échoue à priori automatiquement.
Dans un tel cas, un personnage a néanmoins deux ultimes recours :

* il peut utiliser l'<<attribute_edge,Edge>> pour se donner des dés supplémentaires et surmonter la difficulté.
* le MJ peut autoriser le personnage à lancer un unique dé. Si ce dé fait 5 ou 6, l'action réussit ... tout juste !
...
Pour cela, il faut redonner à l'équipement son véritable rôle :
...
Tous les personnages commencent avec les attributs spéciaux par défaut <<attribute_edge,Edge>> et <<attribute_essence,Essence>>.
...
* Le concept d'« Essence restante » n'a plus vraiment lieu d'être : câblés comme adeptes utilisent l'intégralité de leur Essence pour <<chapter_augmentation,s'augmenter>>.
  Cependant, si le coût total des <<augmentation_technologic,augmentations technologiques>> d'un personage devient strictement supérieure à son Essence, il meurt.
...
Le coût pour <<chapter_augmentation,augmenter>> ce moniteur unique est le même qu'avec 2 moniteurs.
Le moniteur de condition unique ne peut toujours être augmenté que de 3 cases au maximum.

(*TODO*) _Illustrer ça avec des dessins!_

*Motivation :* Cette règle propose d'accélérer le jeu aussi bien en augmentant la lethalité des combats
(les personnages ont moins de cases de blessures) et en simplifiant la gestion des blessures
en rendant le calcul du modificateur dû aux blessures plus immédiat (une fois qu'on a pris le coup).

Un compromis peut être d'accorder deux moniteurs de conditions aux PJ et PNJ importants,
