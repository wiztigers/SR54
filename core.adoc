...
L'état de santé d'un personnage est représenté par un nombre de cases qui constituent le moniteur de condition.

Le nombre de cases du moniteur de condition est de *9 + <<attribute_charisma,Charisme>>/2 + <<attribute_body,Constitution>>*.
...
*Règle :* Le personnage a deux moniteurs de condition au lieu d'un.
...
*Motivation :* Cette règle  simplifie le marquage des blessures, mais complique un peu le calcul des malus.
Elle diminue par ailleurs la lethalité des combats, étant donné que les personnages ont davantage de cases de blessures.
