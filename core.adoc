...
|Action facilitée        |+1 à +4
|Action inratable        |+5 à +10
|Action malaisée         |-1 à -3
|Action difficile        |-4 à -7
|Action quasi-impossible |-8 à -10
|===
...
*Règle :* Au cours d'un test, après application des modificateurs à une réserve de dés,
le nombre maximal de dés à lancer ne peut jamais dépasser le nombre de dés tel qu'il était _avant_ application des modificateurs.

*Motivation :* Cette règle vise à maîtriser l'inflation des réserves de dés.
Pour cela, il faut rendre à l'équipement son véritable rôle :
rendre les personnages capables d'accomplir des actions en dépit de difficultés parfois extrêmes,
et non pas faire le travail à leur place en leur permettant d'obtenir un bon résultat en dépit de leur incompétence.
====
...
Cela représente un conflit direct entre deux personnages, ou entre un personnage et un drone, etc.
...
* Celui qui obtient le plus de succès est le *gagnant* le conflit.
  Celui qui en obtient le moins est le *perdant* le conflit.
  Si les deux opposant obtiennent le même nombre de succès, il n'y a ni gagnant, ni perdant.
* On retranche au nombre de succès du perdant du nombre de succès du gagnant.
  Le nombre de succès restants représente le nombre de *succès nets* du gagnant.
* On compare ces succès nets avec le *seuil* fixé par le MJ pour déterminer le degré de réussite de l'action
  ou les effets infligés au perdant.
...
*Règle :* Lors d'un test opposé, si l'une des réserves de dés dépasse l'autre de 5 dés ou plus,
seul celui qui a la plus grande réserve lance les dés, et le test opposé devient un test simple.
On retranche à la réserve de dés du lanceur un nombre de dés égal à la réserve de dés de son opposant.
Le nombre de succès obtenus lors de cet unique jet devient le nombre de succès nets du test opposé.
Si le lanceur n'obtient aucun succès, il y a égalité.

*Motivation :* Cette règle vise à accélerer le jeu et à minimiser l'aléa des parties.
Elle facilite aussi la vie du MJ, puisqu'en temps normal il doit jeter les dés lors de la plupart des tests opposés.

Cette règle optionnelle ne devrait être appliqué que lorsque une des réserves dépasse clairement l'autre.
En effet, il peut être frustant de n'avoir _aucune_ chance de vaincre un adversaire qui ne lance qu'un seul dé de plus ...
====
...
Si la réserve a une taille supérieure, on compte un succès automatique pour 4 dés à lancer,
...
Cette règle a pour effet additionnel que la puissance des personnages n'est plus exactement linéaire :
au delà de 12 dés, on n'a plus une moyenne d'un succès pour 3 dés lancés, mais d'un pour 4 ...
...
_Peut-être illustrer ce cas avec l'exemple de ?Tom? le chaman Ours troll en Afrique du Sud dans Nosferatu._
====
...
En même temps, elle donne un coté _old school_ aux combats, puisqu'elle est inspirée de ce qui faisait de SR1 à SR3.
