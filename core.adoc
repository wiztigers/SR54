...
Un attribut peut <<chapter_karma,évoluer>> jusqu'à la valeur de 6.
...
Un attribut spécial peut seulement <<chapter_karma,évoluer grâce au karma>>.
...
Ainsi, lorsqu'un personnage fait évoluer un de ses attributs spéciaux, l'autre peut être amené à baisser.
...
Le <<chapter_karma,karma>> ne peut pas faire évoluer directement ces deux attributs, que cela soit à la création de personnage ou ultérieurement.
...
Un attribut ne peut jamais évoluer au delà de 6, ou 9 s'il est augmenté.

[NOTE.example,caption="Exemple"]
====
Sammie a une agilité de 5, augmentée à 9 grâce à son cyberware.
Après une run particulièrement lucrative, il dépense suffisamment de karma pour faire évoluer son agilité non augmentée à 6.
Son agilité augmentée reste à 9.
====
