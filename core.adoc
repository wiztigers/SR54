...
On peut faire évoluer ces attributs par le karma jusqu'au maximum non-augmenté (6, voir <<chapter_metatypes,Métatypes>> ou <<chapter_karma,Évolution du personnage>>). Ils peuvent aussi être augmentés jusqu'au maximum augmenté (9, voir <<chapter_augmentation,Augmentation>>).
