...
Seul le karma peut faire évoluer la valeur d'un attribut spécial. Un attribut spécial ne peut pas être <<chapter_augmentation,augmenté>>.

Un personnage possédant deux attributs spéciaux mais désirant <<chapter_karma,acquérir un nouvel attribut spécial>> doit forcément en abandonner au moins un autre.
...
* Le nombre de cases du moniteur de condition physique est de *8 + <<attribute_body,Constitution>>*.
...
*TODO* _Reformuler les atouts et handicaps qui le nécessitent_
...
Son nombre de cases est donc : *8 + <<attribute_charisma,Charisme>>/2 + <<attribute_body,Constitution>>*.
